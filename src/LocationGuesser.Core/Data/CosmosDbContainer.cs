...
internal class CosmosDbContainer : ICosmosDbContainer, IDisposable
{
    ...
    public CosmosDbContainer(IOptions<CosmosDbOptions> cosmosDbOptions)
    {
        ...
        _client = new CosmosClient(options.Endpoint, tokenCredential: new DefaultAzureCredential());
    ...
    public Task<ItemResponse<T>> ReadItemAsync<T>(string id, PartitionKey partitionKey, CancellationToken cancellationToken)
    {
